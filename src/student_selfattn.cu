...
//   * brotensor::conv2d_forward_gpu has no `groups` argument as of the
//     current pin (it accepts a single OIHW filter, groups=1). The task
//     spec calls for depthwise conv but explicitly allows a fall-back to a
//     full 3x3 conv when groups isn't available — we do exactly that here.
...
void SelfAttnStudent::allocate(int channels_, int height_, int width_) {
    ...
    for (int i = 0; i < kNumLayers; ++i) {
        // Full 3x3 conv (C_out=C, C_in=C, kH=kW=3) — depthwise emulated.
        dw_w[i].resize(C, C * 3 * 3, bt::Dtype::FP16);
...
void SelfAttnStudent::forward(const bt::GpuTensor& x_nchw,
    ...
    for (int i = 0; i < kNumLayers; ++i) {
        // scratch = dwconv3x3(y) + dw_b   (full 3x3 conv, stride=1, pad=1).
        // brotensor's conv2d_forward_gpu lacks a `groups` arg, so we emulate
        // depthwise with a full C->C conv (slower but functionally equivalent
        // once the off-diagonal channel weights stay near zero in training).
