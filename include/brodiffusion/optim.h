...
// adam_step, then casts the updated FP32 master back into the FP16 weight.
//
...
public:
    ...
    int register_param(brotensor::Tensor& param_fp16, std::string name = "");
    ...
    void accumulate_fp16(int handle, const brotensor::Tensor& dW_fp16);
    void accumulate_fp32(int handle, const brotensor::Tensor& dW_fp32);
    ...
    const brotensor::Tensor& master(int handle) const;

    int step_count() const { return step_n_; }

private:
    struct Slot {
        brotensor::Tensor* p_fp16;  // non-owning
        brotensor::Tensor  p_fp32;
        brotensor::Tensor  g_fp32;
        brotensor::Tensor  m_fp32, v_fp32;
        std::string           name;
    };
