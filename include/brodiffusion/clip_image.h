...
//
// Inference-only, FP16, batch size N = 1. Input is a preprocessed FP16
// activation: (1, 3 * 224 * 224) NCHW, already mean/std normalised in CLIP
// space. clip_score.h owns the host-side resize + normalisation that gets
// you from a [-1, 1] SD output to that input.
...
public:
    ...
    //
    // All loaded as FP16; F32 weights are converted host-side on the fly.
    // If `pre_layrnorm_alt` is true, the loader accepts "pre_layernorm.*" as
    // a fallback for the typo-fixed variant some forks ship.
